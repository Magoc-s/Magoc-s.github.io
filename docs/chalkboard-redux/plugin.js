...
//
const pens = ['white', 'black', 'red', 'green', 'erase'];
...
.toolbar {
	...
	let penForm = pens.map(name => {
		...
		return `<label for="${name}-${toolbarCount}">${name}</label> \n` +
               `<input type="radio" name="pen-${toolbarCount}" id="${name}-${toolbarCount}" value="${name}" ${selected} aria-label="colour selector for pen-${toolbarCount}">`;
	}).join('\n');
...
window.ChalkboardRedux = function() {
	...
	return {
		...
		init: (deck) => {
			...
			let notes = getBoard(width, height, 'red');
